...
class VeDbusService(object):
    def __init__(self, servicename, bus=None):
        ...
        self._dbusname = dbus.service.BusName(
            servicename, self._dbusconn, do_not_queue=True
        )
    ...
    def add_path(
        ...
        logging.debug(
            "added %s with start value %s. Writeable is %s" % (path, value, writeable)
        )
        return item
    ...
    def __delitem__(self, path):
        self._dbusobjects[
            path
        ].__del__()  # Invalidates and then removes the object path
...
class VeDbusRootTracker(object):
    ...
    def __init__(self, bus, serviceName):
        ...
        self._match = bus.get_object(
            serviceName, "/", introspect=False
        ).connect_to_signal("ItemsChanged", weak_functor(self._items_changed_handler))
        ...
        for p, item in self._service._dbusobjects.items():
            if p.startswith(px):
                v = (
                    item.GetText()
                    if get_text
                    else wrap_dbus_value(item.local_get_value())
                )
                r[p[len(px) :]] = v
        logging.debug(r)
        return r
...
class VeDbusItemExport(dbus.service.Object):
    ...
    def _local_set_value(self, newvalue):
        ...
        return {"Value": wrap_dbus_value(newvalue), "Text": self.GetText()}
    ...
    @dbus.service.method(
        "com.victronenergy.BusItem", in_signature="v", out_signature="i"
    )
    def SetValue(self, newvalue):
        ...
        if self._onchangecallback is None or (
            self._onchangecallback is not None
            and self._onchangecallback(self.__dbus_object_path__, newvalue)
        ):
    ...
    # @return description
    @dbus.service.method(
        "com.victronenergy.BusItem", in_signature="si", out_signature="s"
    )
