...
def get_vrm_portal_id():
    ...
    try:
        portal_id = (
            check_output("/sbin/get-unique-id").decode("utf-8", "ignore").strip()
        )
        if not portal_id:
...
def wrap_dbus_value(value):
    ...
    if isinstance(value, dict):
        ...
        return dbus.Dictionary(
            {(k, wrap_dbus_value(v)) for k, v in value.items()}, variant_level=1
        )
    return value
...
def add_name_owner_changed_receiver(
    dbus, name_owner_changed, namespace="com.victronenergy"
):
